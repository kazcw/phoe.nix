...
let
  ...
  cfg = config.services.rtorrent-tmux;

in {
  options.services.rtorrent-tmux = {
    ...
    openFirewall = mkOption {
      ...
      default = false;
      description = ''
        Whether to open the firewall for the port in <option>services.rtorrent-tmux.port</option>.
      '';
    };
  ...
  config = mkIf cfg.enable {
    ...
    services.rtorrent-tmux.configText = mkBefore ''
    ...
    systemd = {
      services = {
        ...
        in {
          ...
          path = [ cfg.package pkgs.bash pkgs.tmux ];
	    ...
	    KillMode = "none";
            ...
            ExecStart="${pkgs.tmux}/bin/tmux new-session -c ${cfg.dataDir} -s rtorrent -n rtorrent -d ${cfg.package}/bin/rtorrent -n -o import=${rtorrentConfigFile} && ${pkgs.tmux}/bin/tmux has-session -t rtorrent";
